CREATE OR REPLACE VIEW loadoutcommentcount AS 
 SELECT loadoutcomments.loadoutid, count(loadoutcomments.commentid) AS count
...
SELECT searchableloadouts.loadoutid, 
    ...
    invgroups.groupname::text AS groups, fittings.evebuildnumber, COALESCE(lcc.count, 0) AS comments
   ...
   LEFT JOIN osmium.loadoutcommentcount lcc ON lcc.loadoutid = searchableloadouts.loadoutid
