CREATE OR REPLACE VIEW loadoutssearchresults AS 
 SELECT loadouts.loadoutid, loadouts.privatetoken, 
    loadoutslatestrevision.latestrevision, loadouts.viewpermission, 
    loadouts.visibility, fittings.hullid, invtypes.typename, 
    fittings.creationdate, loadouthistory.updatedate, fittings.name, 
    fittings.evebuildnumber, accounts.nickname, accounts.apiverified, 
    accounts.charactername, accounts.characterid, accounts.corporationname, 
    accounts.corporationid, accounts.alliancename, accounts.allianceid, 
    loadouts.accountid, fittingaggtags.taglist, accounts.reputation, 
    loadoutupdownvotes.votes, loadoutupdownvotes.upvotes, 
    loadoutupdownvotes.downvotes, COALESCE(lcc.count, 0::bigint) AS comments, 
    lda.dps, lda.ehp, lda.estimatedprice
   FROM loadouts
