CREATE OR REPLACE VIEW loadoutssearchdata AS
	SELECT DISTINCT searchableloadouts.loadoutid, 
		...
		END AS author, loadouts.accountid, fittings.name, fittingdescriptions.descriptions AS description, fittings.hullid AS shipid, invtypes.typename AS ship, fittings.creationdate, loadouthistory.updatedate, ls.upvotes, ls.downvotes, ls.score, invgroups.groupname AS groups
