...
CREATE VIEW typessearchdata AS
 SELECT t.typeid, it.typename, pit.typename AS parenttypename, t.category, t.subcategory, ig.groupname, 
    COALESCE(imt.metagroupid::integer, dta_mg.value::integer, 
        CASE dta_tl.value::integer
            WHEN 2 THEN 2
            WHEN 3 THEN 14
            ELSE 1
        END) AS metagroupid, dta_ml.value::integer as metalevel,
    img.marketgroupid, img.marketgroupname, t.other
